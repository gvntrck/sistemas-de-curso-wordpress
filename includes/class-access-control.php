...
<table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="margin:0; padding:0; background-color:#e8ecf1; font-family:Arial,Helvetica,sans-serif;">
  <tr>
    ...
    public function admin_process()
    {
        ...
        // Matrícula em Lote (Múltiplos Alunos)
        if (isset($_POST['matricular_lote']) && wp_verify_nonce($_POST['_wpnonce'], 'aluno_matricula_lote')) {
            ...
            $curso_id = !empty($_POST['lote_curso_id']) ? (int) $_POST['lote_curso_id'] : 0;
            $trilha_id = !empty($_POST['lote_trilha_id']) ? (int) $_POST['lote_trilha_id'] : 0;
            ...
            if (!empty($user_ids)) {
                ...
                if ($curso_id > 0) {
                    foreach ($user_ids as $uid) {
                        self::grant_access($uid, $curso_id, $data_fim, $manager_id);
                    }
                } elseif ($trilha_id > 0) {
                        ...
                        <select name="filter_role">
                            ...
                            <form method="post" id="form-matricula-lote">
                                ...
                                <div class="alignleft actions bulkactions" style="margin-bottom: 10px;">
                                    <button type="button" class="button action" onclick="abrirModalMatriculaLote()">Matricular Selecionados</button>
                                </div>
                                                    ...
                                                    ?>
                                                            <tr>
                                                                <th scope="row" class="check-column" style="padding: 8px 10px;">
                                                                    <input type="checkbox" name="user_ids[]" value="<?php echo $user->ID; ?>" class="aluno-checkbox">
                                                                </th>
                                                                <td><strong>#
                                                                        <?php echo $user->ID; ?>
                                                                    </strong></td>
                            ...
                            <div id="modal-matricula-lote" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:99999; align-items:center; justify-content:center;">
                                <div style="background:#fff; padding:20px; border-radius:4px; min-width:400px; max-width:90%;">
                                    <h2 style="margin-top:0;">Matrícula em Lote</h2>
                                    <p>Selecione um curso <strong>ou</strong> uma trilha para os alunos selecionados.</p>
                            
                                    <div style="margin-bottom: 15px;">
                                        <label style="display:block; margin-bottom:5px;"><strong>Selecionar Curso:</strong></label>
                                        <select id="lote-curso-id" style="width: 100%;">
                                            <option value="">-- Nenhum (Usar Trilha) --</option>
                                            <?php foreach ($cursos as $curso): ?>
                                    ...
                                    <div style="margin-bottom: 15px;">
                                        <label style="display:block; margin-bottom:5px;"><strong>Selecionar Trilha:</strong></label>
                                        <select id="lote-trilha-id" style="width: 100%;">
                                            <option value="">-- Nenhuma (Usar Curso) --</option>
                                            <?php
                                    ...
                                    <div style="margin-bottom: 15px;">
                                        ...
                                        <p class="description">Deixe em branco para acesso vitalício.</p>
                                    </div>

                                    <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 20px;">
                                        <button type="button" class="button" onclick="document.getElementById('modal-matricula-lote').style.display='none'">Cancelar</button>
                                        <button type="button" class="button button-primary" onclick="confirmarMatriculaLote()">Confirmar Matrícula Múltipla</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <script>
                                ...
                                function confirmarMatriculaLote() {
                                    const form = document.getElementById('form-matricula-lote');
                                    const cursoId = document.getElementById('lote-curso-id').value;
                                    const trilhaId = document.getElementById('lote-trilha-id').value;
                                    const dataFim = document.getElementById('lote-data-fim').value;

                                    if (!cursoId && !trilhaId) {
                                        alert('Você deve selecionar um curso ou uma trilha.');
                                        return;
                                    }
                                    if (cursoId && trilhaId) {
                                        alert('Selecione apenas um curso OU uma trilha, não ambos.');
                                        return;
                                    }

                                    // Adicionar os valores ao formulário e submeter
                                    let inputAcao = document.createElement('input');
                                    inputAcao.type = 'hidden';
                                    inputAcao.name = 'matricular_lote';
                                    inputAcao.value = '1';
                                    ...
                                    if (trilhaId) {
                                        ...
                                        inputTrilha.name = 'lote_trilha_id';
                                        inputTrilha.value = trilhaId;
