 ...
 * - Tabela DB: {prefix}progresso_aluno (criada automaticamente no init)
 *
 * Uso:
 ...
 * - O progresso do aluno (aulas concluídas) é armazenado na tabela customizada "progresso_aluno".
 */
...
/**
 * Atualiza a porcentagem de progresso do usuário no curso
 */
function lista_aulas_atualizar_progresso_usuario($user_id, $curso_id)
{
    if (!$user_id || !$curso_id)
        return false;

    // 1. Total de aulas do curso
    // Assume que a chave do relacionamento é 'curso', conforme padrão do plugin
    $meta_key = 'curso';
    ...
    // Se não tiver aulas, progresso é 0 (ou 100 se considerar curso concluído? Geralmente 0)
    if ($total_aulas == 0) {
    ...
    // Conta quantas aulas do curso específico o aluno concluiu
    ...
    // 4. Salvar no usermeta user_id, key, value
...
function lista_aulas_ajax_toggle_concluida()
{
    ...
    if ($ja_concluida) {
        ...
        lista_aulas_atualizar_progresso_usuario($user_id, $curso_id);
            ...
            <div class="lista-aulas__header">
                ...
                <?php if ($isLoggedIn): ?>
                    <button type="button"
                        ...
                        <span
                            class="lista-aulas__btn-texto"><?php echo $aulaAtualConcluida ? 'Concluído' : 'Marcar como concluído'; ?></span>
                    </button>
                <?php endif; ?>
            </div>
